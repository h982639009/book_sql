...
public class SQL {
	...
	final int DIALOG_SEARCH_BOOK = 3;
	final int DIALOG_SEARCH_PRINTER = 4;
	...
	public void initTableData() {
		...
		for (int i = 0; i < strings_printers.length; i++) {
			// 不生成表达式
			insertData_printer(bools_printers, strings_printers[i], SQLUtils.GENE_EXPR_N);
		}
	...
	// search_order 升序或降序
	public Cursor search_book(boolean[] bools, String[] strings, int search_Mode, int link_condition_op,
			int order_or_not, int search_column, int search_order, int expr) {
		// 首先需要根据查询条件构建查询表达式
		...
		if (bools.length != strings.length && bools.length != SQLUtils.table_book_column.length) {
			return null;
		}
		int and_flag = 0;
			...
			if (bools[i] == true) {
				if (and_flag == 0) {
					if (i == 2 || i == 4) {// 年份和页数要单独处理
						...
						if (sub.length == 2) {
							// 填了两项，使用between
							searchSQL += " " + SQLUtils.table_book_column[i] + " between " + sub[0] + " and " + sub[1];

						} else {
							searchSQL += " " + SQLUtils.table_book_column[i] + " = " + sub[0];
						}
					} else {
						searchSQL += " " + SQLUtils.table_book_column[i] + link_op
					...
					if (i == 2 || i == 4) {// 年份和页数要单独处理
						...
						if (sub.length == 2) {
							// 填了两项，使用between
							searchSQL += link_con_op + SQLUtils.table_book_column[i] + " between " + sub[0] + " and "
									+ sub[1];

						} else {
							searchSQL += link_con_op + SQLUtils.table_book_column[i] + " = " + sub[0];
						}
					} else {
						searchSQL += link_con_op + SQLUtils.table_book_column[i] + link_op
								+ SQLUtils.quotationed(strings[i], search_Mode);
					}
				}
			}
	...
	public Cursor search_printer(boolean[] bools, String[] strings, int search_mode, int link_condition_op,
			int order_or_not, int search_column, int search_order,int expr) {
		// 首先需要根据查询条件构建查询表达式
		...
		if (bools.length != strings.length && bools.length != SQLUtils.table_printer_column.length) {
			return null;
		}
		int and_flag = 0;
		for (int i = 0; i < bools.length; i++) {
			if (bools[i] == true) {
				if (and_flag == 0) {
					searchSQL += " " + SQLUtils.table_printer_column[i] + link_op
							+ SQLUtils.quotationed(strings[i], search_mode);
					and_flag = 1;
				} else {
					searchSQL += link_con_op + SQLUtils.table_printer_column[i] + link_op
							+ SQLUtils.quotationed(strings[i], search_mode);
				}
			}
		}
		...
		if (expr == SQLUtils.GENE_EXPR_Y) {
			showDialog(searchSQL, DIALOG_SEARCH_PRINTER);
		}else{
			try {
				cursor = db.rawQuery(searchSQL, null);
				Utils.showToast(context, "搜索出版社表成功啦！~");
				startResultActivity_Printer();
			} catch (Exception e) {
	...
	public boolean insertData_printer(boolean[] bools, String[] strings, int expr) {
	...
	public boolean delete_printer(boolean[] bools, String[] strings,int expr) {
		String deletesql = "delete from " + SQLUtils.tableName_printer + " where ";
		int and_flag = 0;
		for (int i = 0; i < bools.length; i++) {
			if (bools[i] == true) {
				if (and_flag == 0) {
					deletesql += " " + SQLUtils.table_printer_column[i] + "=" + SQLUtils.quotationed(strings[i]);
					and_flag = 1;
				} else {
					deletesql += " and " + SQLUtils.table_printer_column[i] + "=" + SQLUtils.quotationed(strings[i]);
				}
			}
		}
	...
	public boolean update_book(int columnindex, String str, String primarykey, int expr) {
		String updatesql = "update " + SQLUtils.tableName_book + " set " + SQLUtils.table_book_column[columnindex]
				+ " = " + SQLUtils.quotationed(str) + " where title=" + SQLUtils.quotationed(primarykey);
	...
	// 将columnindex对应列（从0开始计数）的属性值修改为str
	public boolean update_printer(int columnindex, String str, String primarykey, int expr) {
		String updatesql = "update " + SQLUtils.tableName_printer + " set " + SQLUtils.table_printer_column[columnindex]
				+ " = " + SQLUtils.quotationed(str) + " where printer=" + SQLUtils.quotationed(primarykey);
	...
	public boolean update_printer(int columnindex, String str, String primarykey){ 
		//默认不产生表达式
		update_printer(columnindex, str, primarykey, SQLUtils.GENE_EXPR_N);
		return false;
	}
	...
	public void showDialog(final String sql, final int flag) {
		...
		builder.setCancelable(false).setTitle("您将要执行如下sql语句").setView(dialog)
...
//								mainActivity_context.startActivity(intent);
	...
	void startResultActivity_Printer(){
...
//			strList.add(cursor.getString(1));
			...
			strList.add(cursor.getString(cursor.getColumnIndex(SQLUtils.table_printer_column[0])));
			strList.add(cursor.getString(cursor.getColumnIndex(SQLUtils.table_printer_column[0])));
